...
#class password_valid(x):
 #      pattern = r'^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*#?&])[A-Za-z\d@$!%*#?&]{,}$'
 #      if re.match(pattern, x) is None:
 #            raise print('Password has incorrect format.')
...
def list_users(res):
    ...
    for i in res:
        ...
        dict = {'email': valid.email, 'password': valid.password, 'gender': valid.gender, 'name_title': valid.name_title,
                ...
                'postcode': valid.postcode, 'latitude': valid.latitude, 'longitude': valid.longitude, 'username': valid.username, 'age': valid.age,

...
def database_creation(connection):
    with connection().cursor() as cursor:
            ...
            query = f'''CREATE TABLE IF NOT EXISTS registration_data2 (
         user_id int PRIMARY KEY, email VARCHAR(255), username VARCHAR(255), password VARCHAR(255), password_md5 VARCHAR(255), password_validation bool, dt TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
         FOREIGN KEY (user_id)
         REFERENCES users2 (user_id)
             ON DELETE cascade)'''
            cursor.execute(query)

            query = f'''CREATE TABLE IF NOT EXISTS media_data2 (
         ...
         REFERENCES users2 (user_id)
            ...
            query = f'''CREATE TABLE IF NOT EXISTS contact_details2 (
         ...
         REFERENCES users2 (user_id)
            ...
            query = f'''CREATE TABLE IF NOT EXISTS locations2 (
         ...
         REFERENCES users2 (user_id)
            ...
            query = f'''CREATE TABLE IF NOT EXISTS cities2 (
